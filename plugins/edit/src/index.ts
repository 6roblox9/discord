...
const performSilentEdit = async (channelId: string, messageId: string, reqData: any, orig: any) => {
    try {
        ...
        if (!msgArray || !msgArray.length) return orig(...[channelId, messageId, reqData]);

        const msg = msgArray.find((m: any) => m.id === messageId);
        if (!msg) return orig(...[channelId, messageId, reqData]);
        ...
        return orig(...[channelId, messageId, reqData]);
    }
};

export default {
    onLoad() {
        if (storage.injectOriginalEdit) {
            unpatchEditMessage = instead("editMessage", MessageActions, async (args, orig) => {
                const [channelId, messageId, reqData] = args;
                
                if (pendingSilentEditMessageId === messageId) {
                    pendingSilentEditMessageId = null;
                    return await performSilentEdit(channelId, messageId, reqData, orig);
                }
                    ...
                    if (storage.injectOriginalEdit) {
                        ...
                        const silentEditBtn = React.createElement(ActionSheetRow, {
                            ...
                            onPress: () => {
                                ...
                                const editRow = targetGroupChildren![targetIndex];
                        ...
                        targetGroupChildren.splice(targetIndex, 0, silentEditBtn);
                    }
                });
            });
        });
